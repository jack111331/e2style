...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, conditions=None, force_fp32=False, fused_modconv=None, **layer_kwargs):
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, conditions=None, **block_kwargs):
        block_ws = []
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            if res == 64 and conditions is not None:
                x, img = block(x, img, cur_ws, conditions=conditions, **block_kwargs)
            else:
